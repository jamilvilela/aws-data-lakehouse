...
**Partitioning Strategy:** Delta Lake tables are partitioned by `event_date` (date) — `fr_flights` derives it from `scheduled_departure` (`%Y-%m-%d`) — except `fr_aircraft_positions`, which is partitioned by `aircraft_icao24` (string). Parquet tables use `reference_date` (date).
