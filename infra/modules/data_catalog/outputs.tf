...
output "aws_glue_catalog_database_trusted_name" {
  description = "Name of the Glue Catalog trusted database"
  value       = aws_glue_catalog_database.db_trusted.name
}
...
output "aws_glue_catalog_table_dataquality_metrics_name" {
  ...
  value       = module.db_raw.tables["data_quality"]
}